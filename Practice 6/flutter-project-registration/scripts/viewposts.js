(() => {
	...
	newPost.addEventListener('submit', event => {
		...
		const tweetMessage = document.getElementById('new-post-text').value;
		window.tweet.post(tweetMessage);
		tweetMessage.value = "";
	});
	...
	tweetsDB.on('child_added', data => {
		...
		postContainer.prepend(content);

		content.querySelector('.like-btn').addEventListener('click', event => {
			tweet.incrementLikes(event.target.getAttribute('data-id'));
			event.preventDefault();
		})

		content.querySelector('.dislike-btn').addEventListener('click', event => {
			tweet.incrementLikes(event.target.getAttribute('data-id'));
			event.preventDefault();
		})

		content.querySelector('.post-close').addEventListener('click', event => {
